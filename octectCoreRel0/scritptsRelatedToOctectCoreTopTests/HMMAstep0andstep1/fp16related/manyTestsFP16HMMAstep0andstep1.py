...
OUTPUT_FILE_HUMAN = os.path.join(SCRIPT_DIR, "hmma_step0andstep1_fp16_tests.txt")
OUTPUT_FILE_TB = os.path.join(SCRIPT_DIR, "hmma_step0andstep1_tb_input.txt")
...
def matrix_to_lines_decimal(mat: np.ndarray) -> str:
    return "\n".join(" ".join(format_fp16_value(v) for v in row) for row in mat)


def matrix_to_lines_hex(mat: np.ndarray) -> str:
    return "\n".join(" ".join(format_fp16_hex(np.float16(v)) for v in row) for row in mat)
...
def generate_safe_C_with_fixed_A_B(rng: np.random.Generator,
    ...
    for _ in range(max_attempts):
        ...
        R = compute_fp16_matmul_add(A_fixed, B_fixed.T, C)
...
def write_human_test_block(f,
    ...
    f.write(f"#{title_word} HMMAstep0/step1 test related values(FP16)\n")
...
def write_tb_test_block(f,
                        ...
                        C01: np.ndarray,
                        C11: np.ndarray):
    f.write(f"#{ordinal_word(test_idx)} TEST VALUES OF HMMAstep0andstep1 (FP16)\n")
...
def main():
    ...
    with open(OUTPUT_FILE_HUMAN, "w", encoding="utf-8") as f_human, \
        ...
        for test_idx in range(1, NUM_TESTS + 1):
            # Step0-related coherent data
            A00, B00, C00, D00 = generate_safe_triplet(rng)
            A10, C10, D10 = generate_safe_aux_pair_with_fixed_B(rng, B00)

            # Step1-related coherent data
            # A00 and A10 stay fixed
            # B01 shared between D01 and D11
            # C01 and C11 searched until safe
