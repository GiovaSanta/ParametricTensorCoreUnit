        ...
        for token, label in zip(tokens, LABELS):
            if len(token) != 8:
                raise ValueError(
                    ...
                    f"eight hex digits, found {token!r}."
                )
...
def main():
    ...
    parser.add_argument(
        "input",
        nargs="?",
        default="generated_fp32_vectors_100/fp32_vectors.txt",
    )
    parser.add_argument(
        "--output-dir",
        default="fp32_input_faults_100",
    )
                ...
                for bit in range(WIDTH_BITS):
                    for stuck_at in (0, 1):
                        ...
                        writer.writerow([
                            ...
                            bit,
                            stuck_at,
                            f"{original_values[operand_index]:08X}",
                            f"{faulty_values[operand_index]:08X}",
                            (
