# FP8 100-vector input stuck-at campaign
#
# Expected location:
#   Tesi/DPU_Fault_InjectionWork/100vectors/
#   script3_run_fp8_fault_campaign_100vectors.tcl
...
set FMA_FILE [
    file join \
        ...
        FMA_FP8e4m3flopoco.vhd
]

set DPU_FILE [
    file join \
        $RTL_DIR \
        DPU_FP8e4m3flopoco.vhd
]

set TB_FILE [
    file join \
        $CAMPAIGN_DIR \
        FP8_fault_campaign_100vectors_TB.vhd
]

set GOLDEN_INPUT [
    file join \
        $CAMPAIGN_DIR \
        generated_fp8_vectors_100 \
        fp8_vectors.txt
]
...
puts ""
puts "Checking FP8 100-vector campaign paths..."
...
set_property library fp8_lib [get_files $FMA_FILE]
set_property library fp8_lib [get_files $DPU_FILE]

set_property file_type {VHDL 2008} [get_files $FMA_FILE]
set_property file_type {VHDL 2008} [get_files $DPU_FILE]
...
set_property \
    top FP8_fault_campaign_100vectors_TB \
    [get_filesets sim_1]
...
puts ""
puts "FP8 100-vector campaign completed."
