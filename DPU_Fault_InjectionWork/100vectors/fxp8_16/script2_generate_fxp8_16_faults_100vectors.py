    ...
    for line_number, line in enumerate(
        ...
        if len(tokens) != len(OPERANDS):
            raise ValueError(
                ...
                f"{len(OPERANDS)} hexadecimal operands, "
                f"found {len(tokens)}."
            )
...
def format_vector(values):
    ...
    for value, (_, width) in zip(values, OPERANDS):
        tokens.append(f"{value:0{width // 4}X}")
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="fxp8_16_input_faults_100",
    )
                ...
                for bit in range(width):
                    for stuck_at in (0, 1):
                        ...
                        writer.writerow([
                            ...
                            bit,
                            stuck_at,
                            f"{original_values[operand_index]:0{digits}X}",
                            f"{faulty_values[operand_index]:0{digits}X}",
                            (
