...
def read_hex16_lines(path):
    ...
    for line_number, value in enumerate(values, start=1):
        if not HEX16.fullmatch(value):
            raise ValueError(
                f"{path}, line {line_number}: expected a "
                f"four-digit hexadecimal result, found {value!r}."
            )

    return values


def fp16_kind(hex_value):
    raw = int(hex_value, 16)
    exponent = (raw >> 10) & 0x1F
    fraction = raw & 0x03FF

    if exponent == 0x1F:
        return "NaN" if fraction != 0 else "Infinity"
...
parser.add_argument(
    "--manifest",
    default="fp16_input_faults_100/fault_manifest.csv",
)
parser.add_argument(
    "--golden",
    default="fp16_results_100/golden_results.txt",
)
parser.add_argument(
    "--faulty",
    default="fp16_results_100/faulty_results.txt",
)
parser.add_argument(
    "--classification",
    default="fp16_results_100/fault_classification.csv",
)
parser.add_argument(
    "--overall-summary",
    default="fp16_results_100/fault_summary_overall.csv",
)
parser.add_argument(
    "--vector-summary",
    default="fp16_results_100/fault_summary_by_vector.csv",
)
parser.add_argument(
    "--site-summary",
    default="fp16_results_100/fault_summary_by_operand_bit.csv",
)
...
for row, faulty in zip(manifest_rows, faulty_values):
    ...
    if faulty_kind == "NaN":
        overall["faulty_nan_results"] += 1
    elif faulty_kind == "Infinity":
        overall["faulty_infinity_results"] += 1
    elif faulty_kind == "Subnormal":
        overall["faulty_subnormal_results"] += 1
...
for vector_id in vector_ids:
    ...
    vector_rows.append({
        "vector_id": vector_id,
        "golden": golden_values[vector_id],
        "golden_kind": fp16_kind(golden_values[vector_id]),
...
print(f"Faulty NaN results:{overall['faulty_nan_results']:>7}")
print(f"Faulty Inf results:{overall['faulty_infinity_results']:>7}")
