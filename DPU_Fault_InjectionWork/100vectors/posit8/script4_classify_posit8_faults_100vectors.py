    ...
    for line_number, value in enumerate(values, start=1):
        if not HEX8.fullmatch(value):
            raise ValueError(
                ...
                f"expected a two-digit hexadecimal result, "
                f"found {value!r}."
            )
...
parser.add_argument(
    "--manifest",
    default="posit8_input_faults_100/fault_manifest.csv",
)
parser.add_argument(
    "--golden",
    default="posit8_results_100/golden_results.txt",
)
parser.add_argument(
    "--faulty",
    default="posit8_results_100/faulty_results.txt",
)
parser.add_argument(
    "--classification",
    default="posit8_results_100/fault_classification.csv",
)
parser.add_argument(
    "--overall-summary",
    default="posit8_results_100/fault_summary_overall.csv",
)
parser.add_argument(
    "--vector-summary",
    default="posit8_results_100/fault_summary_by_vector.csv",
)
parser.add_argument(
    "--site-summary",
    default="posit8_results_100/fault_summary_by_operand_bit.csv",
)
args = parser.parse_args()

golden_values = read_hex_lines(args.golden)
faulty_values = read_hex_lines(args.faulty)
