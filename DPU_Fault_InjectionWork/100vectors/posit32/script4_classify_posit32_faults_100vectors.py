...
parser.add_argument(
    "--manifest",
    default="posit32_input_faults_100/fault_manifest.csv",
)
parser.add_argument(
    "--golden",
    default="posit32_results_100/golden_results.txt",
)
parser.add_argument(
    "--faulty",
    default="posit32_results_100/faulty_results.txt",
)
parser.add_argument(
    "--classification",
    default="posit32_results_100/fault_classification.csv",
)
parser.add_argument(
    "--overall-summary",
    default="posit32_results_100/fault_summary_overall.csv",
)
parser.add_argument(
    "--vector-summary",
    default="posit32_results_100/fault_summary_by_vector.csv",
)
parser.add_argument(
    "--site-summary",
    default=(
        "posit32_results_100/"
        "fault_summary_by_operand_bit.csv"
    ),
)
...
for row, faulty in zip(manifest_rows, faulty_values):
    ...
    if faulty_kind == "NaR":
        overall["faulty_nar_results"] += 1
    elif faulty_kind == "Zero":
        overall["faulty_zero_results"] += 1
...
for vector_id in vector_ids:
    ...
    vector_rows.append({
        "vector_id": vector_id,
        "golden": golden_values[vector_id],
        "golden_kind": posit_kind(golden_values[vector_id]),
...
print(f"Faulty NaR results:  {overall['faulty_nar_results']}")
print(f"Faulty zero results: {overall['faulty_zero_results']}")
