...
begin
    ...
    begin
        ...
        if hmma_step = '0' then
            b_base := 0;   -- step0 uses lanes 0..3 for B this will be fed to all DPUs related to the octect for the particular step)
        else
            b_base := 4;   -- step1 uses lanes 4..7 for B this will be fed to all DPUs related to the octect for the particular step)
        end if;
        
        --TG0 current A row comes from lanes 0..3, selected by exec_step
        for k in 0 to 3 loop 
            A_tg0_out((k+1)*ELEM_W-1 downto k*ELEM_W) <= A_buf(row_idx)(k);
        end loop;
        
        --TG4 current A row comes from lanes 4..7, selected by exec_step
        for k in 0 to 3 loop
            A_tg4_out((k+1)*ELEM_W-1 downto k*ELEM_W) <= A_buf(row_idx+4)(k);
        end loop;
            
        --TG0 current C row
        for k in 0 to 3 loop
            C_tg0_out((k+1)*ELEM_W-1 downto k*ELEM_W) <= C_buf(row_idx)(k);
        end loop;
        
        --TG4 current C row
        for k in 0 to 3 loop
            C_tg4_out((k+1)*ELEM_W-1 downto k*ELEM_W) <= C_buf(row_idx+4)(k);
        end loop;
        
        --B block is stationary and reused across the 4 execution cycles-
        --here we assume lanes 0..3 store the 4 rows of the 4x4 B tile.
        idx := 0;
        for r in 0 to 3 loop
            for c in 0 to 3 loop
                B_blk_out(idx + ELEM_W-1 downto idx) <= B_buf(r + b_base)(c);
                idx := idx + ELEM_W;
            end loop;
        end loop;
    end process;
