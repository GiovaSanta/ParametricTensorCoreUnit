...
def hex16_to_posit16_1(hex_str: str) -> float:
    bits = int(hex_str, 16)
...
def compare_matrix(sim_mat, gold_mat, out, tolerance=0.0):
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            sim_val = hex16_to_posit16_1(sim_hex)
            gold_val = hex16_to_posit16_1(gold_hex)
            ...
            # For posit validation, exact encoded-word equality is the main check.
            # The tolerance is still reported on decoded values for readability.
...
if __name__ == "__main__":
    sim_output_file = (
        ...
        r"\4SetsOfHMMAstep0step1\posit16related"
        r"\hmma_8instr_dualTC_4octects_tb_output_ctrl_posit16.txt"
    )
    ...
    golden_file = (
        ...
        r"\doubleParametricTCUsRel0\doubleParametricTCUsRelatedScripts"
        r"\4SetsOfHMMAstep0step1\posit16related"
        r"\hmma_8instr_dualTC_4octects_posit16_single_experiment.txt"
    )
    ...
    report_file = (
        ...
        r"\doubleParametricTCUsRel0\doubleParametricTCUsRelatedScripts"
        r"\4SetsOfHMMAstep0step1\posit16related"
        r"\hmma_8instr_dualTC_4octects_posit16_validation_report.txt"
    )
    ...
    with open(report_file, "w", encoding="utf-8") as out:
        write_line(out, "HMMA 4-SET STEP0/STEP1 POSIT16,1 VALIDATION REPORT (DUAL TC, 4 OCTECTS)")
        ...
        write_line(out, "Primary check: exact equality of 16-bit posit encoded words.")
        write_line(out)
        ...
        staged_diffs, staged_missing, staged_hex_mismatches = compare_named_matrix(
            "FINAL 16x16 RESULT vs FINAL_CHAINED_16x16_FROM_DUAL_TC_POSIT16",
