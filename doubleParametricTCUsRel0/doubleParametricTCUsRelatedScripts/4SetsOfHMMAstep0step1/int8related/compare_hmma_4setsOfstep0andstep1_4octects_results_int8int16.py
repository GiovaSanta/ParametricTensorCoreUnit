...
def hex16_to_int16(hex_str: str) -> int:
    v = int(hex_str, 16) & 0xFFFF
...
def compare_matrix(sim_mat, gold_mat, out, tolerance=0):
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            sim_val = hex16_to_int16(sim_hex)
            gold_val = hex16_to_int16(gold_hex)
            ...
            if is_diff:
                ...
                write_line(
                    out,
                    f"{r:>3} {c:>3} | "
                    f"{sim_hex:>8} {sim_val:>12d} | "
                    f"{gold_hex:>8} {gold_val:>12d} | "
                    f"{diff:>10d}"
                )
...
if __name__ == "__main__":
    sim_output_file = (
        ...
        r"\4SetsOfHMMAstep0step1\int8related"
        r"\hmma_8instr_dualTC_4octects_tb_output_ctrl_int8.txt"
    )
    ...
    golden_file = (
        ...
        r"\doubleParametricTCUsRel0\doubleParametricTCUsRelatedScripts"
        r"\4SetsOfHMMAstep0step1\int8related"
        r"\hmma_8instr_dualTC_4octects_int8int16_single_experiment.txt"
    )
    ...
    report_file = (
        ...
        r"\doubleParametricTCUsRel0\doubleParametricTCUsRelatedScripts"
        r"\4SetsOfHMMAstep0step1\int8related"
        r"\hmma_8instr_dualTC_4octects_int8int16_validation_report.txt"
    )
    ...
    with open(report_file, "w", encoding="utf-8") as out:
        write_line(out, "HMMA 4-SET STEP0/STEP1 INT8/INT16 VALIDATION REPORT (DUAL TC, 4 OCTECTS)")
        ...
        write_line(out, "Primary check: exact equality of 16-bit result encoded words.")
        write_line(out)
        ...
        staged_diffs, staged_missing, staged_hex_mismatches = compare_named_matrix(
            "FINAL 16x16 RESULT vs FINAL_CHAINED_16x16_FROM_DUAL_TC_INT16",
