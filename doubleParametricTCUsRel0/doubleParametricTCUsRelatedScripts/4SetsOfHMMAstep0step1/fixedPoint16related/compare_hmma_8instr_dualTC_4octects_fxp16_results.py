...
# ============================================================
# FXP32 helpers
# Compared matrices in this validator are D/final results,
# encoded on 32 bits as signed fixed point with:
#   integer bits = 11
#   fractional bits = 20
#
# So decoded_real = signed_int32 / 2^20
# ============================================================
...
def hex32_to_signed_int(hex_str: str) -> int:
    return twos_complement_to_signed(int(hex_str, 16), FXP32_WORD_BITS)


def hex32_to_fxp32_real(hex_str: str) -> float:
    return hex32_to_signed_int(hex_str) / float(1 << FXP32_FRAC_BITS)
...
#
# Expected golden sections:
#   #golden TC0 STEP0_D00_set0 encoded
#   #golden TC1 STEP0_D00_set0 encoded
#   ...
#   #D_full_from_2tc encoded
#   #D_full_one_shot encoded
# ============================================================
...
def parse_golden_file(path: str):
    ...
    if (
        not results["sets"]
        and results["final_16x16_staged"] is None
        and results["final_16x16_ideal"] is None
    ):
        raise ValueError(
...
def compare_matrix(sim_mat, gold_mat, out, tolerance=0.0):
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            sim_val = hex32_to_fxp32_real(sim_hex)
            gold_val = hex32_to_fxp32_real(gold_hex)
            ...
            if diff > tolerance or sim_hex != gold_hex:
                diff_count += 1
                write_line(
                    out,
                    f"{r:>3} {c:>3} | "
...
def compare_sets(sim_data, golden_data, out, tolerance=0.0):
    ...
    for set_id in all_sets:
        ...
        for mat_name in matrix_names:
            ...
            write_line(out, f"\n{mat_name}:")
            local_diff, max_abs, local_hex_mismatches = compare_matrix(
                sim_mat, gold_mat, out, tolerance=tolerance
            )
...
def compare_named_matrix(title, sim_mat, gold_mat, out, tolerance=0.0):
    ...
    diff_count, max_abs, hex_mismatch_count = compare_matrix(
        sim_mat, gold_mat, out, tolerance=tolerance
    )
...
if __name__ == "__main__":
    sim_output_file = (
        ...
        r"\4SetsOfHMMAstep0step1\fixedPoint16related"
        r"\hmma_8instr_dualTC_4octects_tb_output_ctrl_fxp16.txt"
    )
    ...
    golden_file = (
        ...
        r"\doubleParametricTCUsRel0\doubleParametricTCUsRelatedScripts"
        r"\4SetsOfHMMAstep0step1\fixedPoint16related"
        r"\hmma_8instr_dualTC_4octects_fxp16fxp32_single_experiment.txt"
    )
    ...
    report_file = (
        ...
        r"\doubleParametricTCUsRel0\doubleParametricTCUsRelatedScripts"
        r"\4SetsOfHMMAstep0step1\fixedPoint16related"
        r"\hmma_8instr_dualTC_4octects_fxp16_validation_report.txt"
    )
    ...
    with open(report_file, "w", encoding="utf-8") as out:
        write_line(out, "HMMA 4-SET STEP0/STEP1 FXP16/FXP32 VALIDATION REPORT (DUAL TC, 4 OCTECTS)")
        ...
        write_line(out, "Decoded values use signed fixed-point Q11.20 for D/final matrices.")
        write_line(out)
        ...
        staged_diffs, staged_missing, staged_hex_mismatches = compare_named_matrix(
            "FINAL 16x16 RESULT vs FINAL_CHAINED_16x16_FROM_DUAL_TC_FXP32",
