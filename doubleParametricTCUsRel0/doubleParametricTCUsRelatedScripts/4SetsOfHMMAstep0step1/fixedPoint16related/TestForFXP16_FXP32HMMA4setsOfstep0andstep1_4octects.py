...
OUTPUT_FILE_HUMAN = os.path.join(
    SCRIPT_DIR,
    "hmma_8instr_dualTC_4octects_fxp16fxp32_single_experiment.txt"
)
...
# ------------------------------------------------------------
# Fixed-point format
# A and B: signed fixed-point 16-bit, 5 integer bits, 10 frac bits
# C and D: signed fixed-point 32-bit, 11 integer bits, 20 frac bits
#
# Interpreted as:
#   real_value = signed_integer / 2^frac_bits
# ------------------------------------------------------------
AB_WORD_BITS = 16
AB_FRAC_BITS = 10
...
# Representable real ranges
AB_REAL_MIN = -(2 ** (5 - 1))          # -16
AB_REAL_MAX = (2 ** (5 - 1)) - (1 / (2 ** AB_FRAC_BITS))   # 15.9990234375

CD_REAL_MIN = -(2 ** (11 - 1))         # -1024
CD_REAL_MAX = (2 ** (11 - 1)) - (1 / (2 ** CD_FRAC_BITS))  # 1023.9999990463257

# Generation ranges
# You can tighten these later if you want fewer near-overflow cases.
AB_VALUE_MIN = -4.0
AB_VALUE_MAX = 4.0
...
FXP16_MIN_INT = -(1 << 15)
FXP16_MAX_INT = (1 << 15) - 1

FXP32_MIN_INT = -(1 << 31)
FXP32_MAX_INT = (1 << 31) - 1
...
def real_to_fxp16_int(x: float) -> int:
    scaled = int(np.round(float(x) * (1 << AB_FRAC_BITS)))
    return clamp_int(scaled, FXP16_MIN_INT, FXP16_MAX_INT)


def real_to_fxp32_int(x: float) -> int:
    scaled = int(np.round(float(x) * (1 << CD_FRAC_BITS)))
    return clamp_int(scaled, FXP32_MIN_INT, FXP32_MAX_INT)


def fxp16_int_to_real(x: int) -> float:
    return float(int(x)) / float(1 << AB_FRAC_BITS)


def fxp32_int_to_real(x: int) -> float:
    return float(int(x)) / float(1 << CD_FRAC_BITS)


def fxp16_to_hex_from_int(x: int) -> str:
    return f"{(int(x) & 0xFFFF):04X}"


def fxp32_to_hex_from_int(x: int) -> str:
    return f"{(int(x) & 0xFFFFFFFF):08X}"


def fxp16_real_to_hex(x: float) -> str:
    return fxp16_to_hex_from_int(real_to_fxp16_int(x))


def fxp32_real_to_hex(x: float) -> str:
    return fxp32_to_hex_from_int(real_to_fxp32_int(x))


def quantize_fxp16_real(x: float) -> float:
    return fxp16_int_to_real(real_to_fxp16_int(x))


def quantize_fxp32_real(x: float) -> float:
    return fxp32_int_to_real(real_to_fxp32_int(x))


def quantize_matrix_fxp16(mat: np.ndarray) -> np.ndarray:
    out = np.empty(mat.shape, dtype=np.float64)
    for r in range(mat.shape[0]):
        for c in range(mat.shape[1]):
            out[r, c] = quantize_fxp16_real(float(mat[r, c]))
    return out


def quantize_matrix_fxp32(mat: np.ndarray) -> np.ndarray:
    out = np.empty(mat.shape, dtype=np.float64)
    for r in range(mat.shape[0]):
        for c in range(mat.shape[1]):
            out[r, c] = quantize_fxp32_real(float(mat[r, c]))
    return out


def matrix_to_lines_real(mat: np.ndarray, decimals: int = 10) -> str:
    return "\n".join(
        " ".join(f"{float(v):.{decimals}f}" for v in row)
        for row in mat
    )
...
def matrix_to_lines_hex_fxp16(mat: np.ndarray) -> str:
    return "\n".join(
        " ".join(fxp16_real_to_hex(float(v)) for v in row)
        for row in mat
    )
...
def matrix_to_lines_hex_fxp32(mat: np.ndarray) -> str:
    return "\n".join(
        " ".join(fxp32_real_to_hex(float(v)) for v in row)
        for row in mat
    )


def random_fxp16_matrix(
    rng: np.random.Generator,
    low: float = AB_VALUE_MIN,
    high: float = AB_VALUE_MAX,
    shape=(FULL_SIZE, FULL_SIZE)
) -> np.ndarray:
    raw = rng.uniform(low, high, size=shape)
    return quantize_matrix_fxp16(raw)


def random_fxp32_matrix(
    rng: np.random.Generator,
    low: float = C_VALUE_MIN,
    high: float = C_VALUE_MAX,
    shape=(FULL_SIZE, FULL_SIZE)
) -> np.ndarray:
    raw = rng.uniform(low, high, size=shape)
    return quantize_matrix_fxp32(raw)


def is_safe_fxp16_matrix(mat: np.ndarray) -> bool:
    return np.all(mat >= AB_REAL_MIN) and np.all(mat <= AB_REAL_MAX)


def is_safe_fxp32_matrix(mat: np.ndarray) -> bool:
    return np.all(mat >= CD_REAL_MIN) and np.all(mat <= CD_REAL_MAX)


def fxp16_fxp32_matmul_add(A: np.ndarray, B: np.ndarray, C: np.ndarray) -> np.ndarray:
    """
    Golden/reference model:
      A : fxp16 real-valued matrix
      B : fxp16 real-valued matrix
      C : fxp32 real-valued matrix
      D = quantize_fxp32( A @ B + C )
    """
    result = A.astype(np.float64) @ B.astype(np.float64) + C.astype(np.float64)
    return quantize_matrix_fxp32(result)
...
def write_matrix_with_encoded_fxp16(f, name: str, mat: np.ndarray):
    f.write(f"#{name}\n")
    f.write(matrix_to_lines_real(mat) + "\n")
...
def write_set_block(
    ...
    for n, m in [
        ...
        write_matrix_with_encoded_fxp32(f, n, m)
    ...
    for tag, M in [
        ...
        f.write(matrix_to_lines_real(M) + "\n")
        f.write(f"#golden {prefix} {tag}_set{set_idx} encoded\n")
        f.write(matrix_to_lines_hex_fxp32(M) + "\n")
...
def encode_row_fxp32_hex(row: np.ndarray):
    return [fxp32_real_to_hex(float(v)) for v in row]


def pack_row_pair00_fxp32(row: np.ndarray):
    h = encode_row_fxp32_hex(row)
    return h[0], h[1]


def pack_row_pair01_fxp32(row: np.ndarray):
    h = encode_row_fxp32_hex(row)
    return h[2], h[3]
...
def write_tb_matrix_block_fxp32_pair00(f, label: str, mat: np.ndarray, base_lane: int):
    f.write(f"#{label}\n")
    for i in range(4):
        lane_id = base_lane + i
        portA, portB = pack_row_pair00_fxp32(mat[i])
        f.write(f"#lane{lane_id}\n")
        f.write(f"{portA} {portB}\n")


def write_tb_matrix_block_fxp32_pair01(f, label: str, mat: np.ndarray, base_lane: int):
    f.write(f"#{label}\n")
    for i in range(4):
        lane_id = base_lane + i
        portA, portB = pack_row_pair01_fxp32(mat[i])
        f.write(f"#lane{lane_id}\n")
        f.write(f"{portA} {portB}\n")
...
def write_tb_set0_dual_tc_phase_ordered(f, s0_tc0: dict, s0_tc1: dict):
    ...
    f.write("#---- A payloads for both tensor cores (fxp16) ----\n")
    ...
    f.write("#---- B payloads for both tensor cores (fxp16) ----\n")
    for label, mat, base in [
    ...
    f.write("#---- C step0 payloads for both tensor cores (fxp32 pair00) ----\n")
    for label, mat, base in [
        ...
        write_tb_matrix_block_fxp32_pair00(f, label, mat, base)

    f.write("#---- C step0 payloads for both tensor cores (fxp32 pair01) ----\n")
    for label, mat, base in [
        ...
        write_tb_matrix_block_fxp32_pair01(f, label, mat, base)

    f.write("#---- C step1 payloads for both tensor cores (fxp32 pair00) ----\n")
    for label, mat, base in [
    ...
    f.write("#---- C step1 payloads for both tensor cores (fxp32 pair01) ----\n")
    for label, mat, base in [
...
def build_one_tc_safe_experiment(A: np.ndarray, B: np.ndarray, C: np.ndarray, tc_col_base: int):
    ...
    for s in range(4):
        ...
        D00 = fxp16_fxp32_matmul_add(A00, B_left_full, C00)
        D10 = fxp16_fxp32_matmul_add(A10, B_left_full, C10)
        D01 = fxp16_fxp32_matmul_add(A00, B_right_full, C01)
        D11 = fxp16_fxp32_matmul_add(A10, B_right_full, C11)
        D20 = fxp16_fxp32_matmul_add(A20, B_left_full, C20)
        D30 = fxp16_fxp32_matmul_add(A30, B_left_full, C30)
        D21 = fxp16_fxp32_matmul_add(A20, B_right_full, C21)
        D31 = fxp16_fxp32_matmul_add(A30, B_right_full, C31)
    ...
    final_16x8 = quantize_matrix_fxp32(final_16x8)
...
def build_dual_tc_safe_experiment(rng: np.random.Generator, max_attempts: int = 20000):
    for _ in range(max_attempts):
        A = random_fxp16_matrix(rng)
        B = random_fxp16_matrix(rng)
        C = random_fxp32_matrix(rng)

        D_full_one_shot = fxp16_fxp32_matmul_add(A, B, C)
        if not is_safe_fxp32_matrix(D_full_one_shot):
            continue
        ...
        D_full_from_2tc = quantize_matrix_fxp32(D_full_from_2tc)

        if not is_safe_fxp32_matrix(D_full_from_2tc):
            continue
    ...
    raise RuntimeError("Could not generate a safe dual-TC fxp16/fxp32 16x16 experiment.")
...
def write_human_file(f, exp):
    f.write("#Full 16x16 GEMM-ACC decomposed across 2 tensor cores, each with 2 octects (A/B=fxp16, C/D=fxp32)\n")
    f.write("#Golden math computes D = quantize_fxp32(A @ B + C).\n")
    f.write("#Only the B payload written to the TB file is transposed for hardware feeding.\n\n")

    write_matrix_with_encoded_fxp16(f, "FULL_A_16x16", exp["A"])
    write_matrix_with_encoded_fxp16(f, "FULL_B_16x16", exp["B"])
    write_matrix_with_encoded_fxp32(f, "FULL_C_16x16", exp["C"])
    write_matrix_with_encoded_fxp32(f, "FULL_D_16x16_one_shot_reference", exp["D_full_one_shot"])
    ...
    f.write("\n#FINAL_CHAINED_16x16_FROM_DUAL_TC_FXP32\n")
    f.write("#D_full_from_2tc decoded\n")
    f.write(matrix_to_lines_real(exp["D_full_from_2tc"]) + "\n")
    f.write("#D_full_from_2tc encoded\n")
    f.write(matrix_to_lines_hex_fxp32(exp["D_full_from_2tc"]) + "\n")

    f.write("\n#FINAL_IDEAL_16x16_ONE_SHOT_REFERENCE\n")
    f.write("#D_full_one_shot decoded\n")
    f.write(matrix_to_lines_real(exp["D_full_one_shot"]) + "\n")
    f.write("#D_full_one_shot encoded\n")
    f.write(matrix_to_lines_hex_fxp32(exp["D_full_one_shot"]) + "\n")


def write_tb_file(f, exp):
    f.write("#register file ports content for mixed-precision FXP16/FXP32 dual tensor-core wrapper\n")
