...
def hex32_to_posit32(hex_str: str) -> float:
    bits = int(hex_str, 16)
...
def compare_matrix(sim_mat, gold_mat, out, tolerance=0.0):
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            sim_val = hex32_to_posit32(sim_hex)
            gold_val = hex32_to_posit32(gold_hex)
            ...
            if is_diff:
                ...
                write_line(
                    out,
                    f"{r:>3} {c:>3} | "
                    f"{sim_hex:>10} {sim_val:>16.8g} | "
                    f"{gold_hex:>10} {gold_val:>16.8g} | "
                    f"{diff:>16.8g}"
                )
...
if __name__ == "__main__":
    sim_output_file = (
        ...
        r"\4SetsOfHMMAstep0step1\posit32related"
        r"\hmma_8instr_dualTC_4octects_tb_output_ctrl_posit32.txt"
    )
    ...
    golden_file = (
        ...
        r"\doubleParametricTCUsRel0\doubleParametricTCUsRelatedScripts"
        r"\4SetsOfHMMAstep0step1\posit32related"
        r"\hmma_8instr_dualTC_4octects_posit32_single_experiment.txt"
    )
    ...
    report_file = (
        ...
        r"\doubleParametricTCUsRel0\doubleParametricTCUsRelatedScripts"
        r"\4SetsOfHMMAstep0step1\posit32related"
        r"\hmma_8instr_dualTC_4octects_posit32_validation_report.txt"
    )
    ...
    with open(report_file, "w", encoding="utf-8") as out:
        write_line(out, f"HMMA 4-SET STEP0/STEP1 POSIT32,{POSIT_ES} VALIDATION REPORT (DUAL TC, 4 OCTECTS)")
        ...
        write_line(out, "Primary check: exact equality of 32-bit posit encoded words.")
        write_line(out)
        ...
        staged_diffs, staged_missing, staged_hex_mismatches = compare_named_matrix(
            "FINAL 16x16 RESULT vs FINAL_CHAINED_16x16_FROM_DUAL_TC_POSIT32",
