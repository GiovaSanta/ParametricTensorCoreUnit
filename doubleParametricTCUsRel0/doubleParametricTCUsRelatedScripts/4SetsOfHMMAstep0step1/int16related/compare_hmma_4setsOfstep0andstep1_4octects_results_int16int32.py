...
def hex32_to_int32(hex_str: str) -> int:
...
def parse_sim_output(path: str):
    ...
    while i < len(lines):
        ...
        if line.upper() in ("#FINAL_16X16_RESULT", "#FINAL_16x16_RESULT".upper()):
...
def parse_golden_file(path: str):
    lines = Path(path).read_text(encoding="utf-8").splitlines()
    results = {"sets": {}, "final_16x16_staged": None, "final_16x16_ideal": None}
    i = 0
...
def compare_matrix(sim_mat, gold_mat, out, tolerance=0):
    rows = len(sim_mat)
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            sim_val = hex32_to_int32(sim_hex)
            gold_val = hex32_to_int32(gold_hex)
            ...
            if is_diff:
                ...
                write_line(
                    out,
                    f"{r:>3} {c:>3} | "
                    f"{sim_hex:>12} {sim_val:>12d} | "
                    f"{gold_hex:>12} {gold_val:>12d} | "
                    f"{diff:>10d}"
                )
...
def compare_sets(sim_data, golden_data, out, tolerance=0):
    ...
    for set_id in all_sets:
        ...
        for mat_name in matrix_names:
            ...
            if local_diff != 0:
                ...
                write_line(out, f"--> max abs diff in {mat_name}: {max_abs}")

    return total_diff_count, total_missing_sets, total_missing_matrices, total_hex_mismatches


def compare_named_matrix(title, sim_mat, gold_mat, out, tolerance=0):
    write_line(out, f"\n========== {title} ==========")
    ...
    if diff_count != 0:
        ...
        write_line(out, f"--> max abs diff: {max_abs}")
    else:
...
if __name__ == "__main__":
    sim_output_file = (
        ...
        r"\4SetsOfHMMAstep0step1\int16related"
        r"\hmma_8instr_dualTC_4octects_tb_output_ctrl_int16.txt"
    )
    ...
    golden_file = (
        ...
        r"\doubleParametricTCUsRel0\doubleParametricTCUsRelatedScripts"
        r"\4SetsOfHMMAstep0step1\int16related"
        r"\hmma_8instr_dualTC_4octects_int16int32_single_experiment.txt"
    )
    ...
    report_file = (
        ...
        r"\doubleParametricTCUsRel0\doubleParametricTCUsRelatedScripts"
        r"\4SetsOfHMMAstep0step1\int16related"
        r"\hmma_8instr_dualTC_4octects_int16int32_validation_report.txt"
    )
    ...
    with open(report_file, "w", encoding="utf-8") as out:
        write_line(out, "HMMA 4-SET STEP0/STEP1 INT16/INT32 VALIDATION REPORT (DUAL TC, 4 OCTECTS)")
        ...
        write_line(out, "Primary check: exact equality of 32-bit result encoded words.")
        write_line(out)
        ...
        staged_diffs, staged_missing, staged_hex_mismatches = compare_named_matrix(
            "FINAL 16x16 RESULT vs FINAL_CHAINED_16x16_FROM_DUAL_TC_INT32",
