#!/usr/bin/env python3
"""
Patch Klessydra TCUopFP32.c from a generated 16x16 FP32 experiment file.
...
Klessydra FP32 C benchmark convention:
...
so it handles fp32_t / volatile fp32_t spacing robustly.
"""
...
def normalize_hex32(token: str) -> str:
    ...
    if not HEX32_RE.fullmatch(token):
        raise ValueError(f"Invalid FP32 hex token: {token!r}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Patch Klessydra FP32 C matrices from generated experiment.")
    ...
    print("Patched Klessydra FP32 C matrices.")
