#!/usr/bin/env python3
"""
Patch Klessydra TCUopFIXED8_16.c from a generated 16x16 FXP8_16 experiment file.

Expected experiment file sections:
    #FULL_A_16x16 encoded                                  -> 8-bit A
    #FULL_B_16x16 encoded                                  -> 8-bit B
    #FULL_C_16x16 encoded                                  -> 16-bit C
    #FULL_D_16x16_full_precision_real_reference_rounded_fxp16 encoded
       preferred golden D reference for validation/reporting
    fallback:
    #FULL_D_16x16_one_shot_reference encoded

Klessydra FXP8_16 C benchmark convention:
    A   is 8-bit and stored row-major.
    B_T is 8-bit and stored as B transpose.
    C   is 16-bit and stored row-major.
    D   is 16-bit, zero-initialized, and filled by the HMMA assembly code.

This patcher detects the actual declaration prefix of A, B_T, C, and D,
so it handles fixed8_tcu_t / fixed16_tcu_t / volatile spacing robustly.
"""
...
PREFERRED_D_LABEL = "#FULL_D_16x16_full_precision_real_reference_rounded_fxp16 encoded"
...
def zeros_matrix(bits: int, rows: int = 16, cols: int = 16) -> List[List[str]]:
    zero = "0x00" if bits == 8 else "0x0000"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Patch Klessydra FXP8_16 C matrices from generated experiment.")
    ...
    A = read_matrix_after_section(args.experiment_file, "#FULL_A_16x16 encoded", bits=8)
    B = read_matrix_after_section(args.experiment_file, "#FULL_B_16x16 encoded", bits=8)
    C = read_matrix_after_section(args.experiment_file, "#FULL_C_16x16 encoded", bits=16)

    d_label = choose_golden_d_label(args.experiment_file)
    D_golden = read_matrix_after_section(args.experiment_file, d_label, bits=16)

    B_T = transpose_matrix(B)
    D_zero = zeros_matrix(bits=16)
    ...
    print("Patched Klessydra FXP8_16 C matrices.")
    ...
    print("A:   8-bit row-major")
    print("B_T: 8-bit transpose of FULL_B_16x16")
    print("C:   16-bit row-major")
    print("D:   16-bit zero-initialized")
