#!/usr/bin/env python3
"""
Patch Klessydra TCUopPOSIT8.c from a generated 16x16 Posit8 experiment file.
...
Klessydra Posit8 C benchmark convention:
...
so it handles posit8_t / volatile posit8_t spacing robustly.
"""
...
def normalize_hex8(token: str) -> str:
    ...
    if not HEX8_RE.fullmatch(token):
        raise ValueError(f"Invalid Posit8 hex token: {token!r}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Patch Klessydra Posit8 C matrices from generated experiment.")
    ...
    print("Patched Klessydra Posit8 C matrices.")
