#!/usr/bin/env python3
"""
Patch Klessydra TCUopPOSIT16.c from a generated 16x16 Posit16 experiment file.
...
Klessydra Posit16 C benchmark convention:
...
def normalize_hex16(token: str) -> str:
    ...
    if not HEX16_RE.fullmatch(token):
        raise ValueError(f"Invalid Posit16 hex token: {token!r}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Patch Klessydra Posit16 C matrices from generated experiment.")
    ...
    source = replace_c_array(
        ...
        "posit16_t",
        "A",
        c_matrix_initializer("posit16_t", "A", "[N_ROW_1][N_COL_1]", A, "//A is stored in row major layout"),
    )
    ...
    source = replace_c_array(
        ...
        "posit16_t",
        "B_T",
        c_matrix_initializer("posit16_t", "B_T", "[N_COL_2][N_COL_1]", B_T, "//B is stored in column major layout in memory (so this below is B transpose)"),
    )
    ...
    source = replace_c_array(
        ...
        "posit16_t",
        "C",
        c_matrix_initializer("posit16_t", "C", "[N_ROW_1][N_COL_2]", C, "// C is stored in row major layout in memory"),
    )
    ...
    source = replace_c_array(
        ...
        "volatile posit16_t",
        "D",
        c_matrix_initializer("volatile posit16_t", "D", "[N_ROW_1][N_COL_2]", D_zero, "// D is stored in row-major layout in memory"),
    )
    ...
    print("Patched Klessydra Posit16 C matrices.")
