#!/usr/bin/env python3
"""
Patch Klessydra TCUopPOSIT32.c from a generated 16x16 Posit32 experiment file.
...
Klessydra Posit32 C benchmark convention:
...
so it handles posit32_t / volatile posit32_t spacing robustly.
"""
...
def normalize_hex32(token: str) -> str:
    ...
    if not HEX32_RE.fullmatch(token):
        raise ValueError(f"Invalid Posit32 hex token: {token!r}")
...
def read_matrix_after_section(path: Path, label: str, rows: int = 16, cols: int = 16) -> List[List[str]]:
    ...
    for raw in lines[start:]:
        ...
        for tok in stripped.split():
            tokens.append(normalize_hex32(tok))
...
def zeros_matrix(rows: int = 16, cols: int = 16) -> List[List[str]]:
    return [["0x00000000" for _ in range(cols)] for _ in range(rows)]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Patch Klessydra Posit32 C matrices from generated experiment.")
    ...
    print("Patched Klessydra Posit32 C matrices.")
