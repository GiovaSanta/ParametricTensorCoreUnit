...
def quantize_real_matrix_to_fxp16_raw(mat: np.ndarray) -> np.ndarray:
    out = np.empty(mat.shape, dtype=np.int32)
    for r in range(mat.shape[0]):
        for c in range(mat.shape[1]):
            out[r, c] = real_to_fxp_raw_sat(float(mat[r, c]), FXP16_FRAC_BITS, 16)
    return out
...
def random_fxp8_matrix_with_real(rng: np.random.Generator, shape=(FULL_SIZE, FULL_SIZE)):
    vals = rng.uniform(AB_REAL_MIN, AB_REAL_MAX, size=shape).astype(np.float64)
...
def random_fxp16_matrix_with_real(rng: np.random.Generator, shape=(FULL_SIZE, FULL_SIZE)):
    vals = rng.uniform(C_REAL_MIN, C_REAL_MAX, size=shape).astype(np.float64)
...
def random_fxp8_matrix(rng: np.random.Generator, shape=(FULL_SIZE, FULL_SIZE)) -> np.ndarray:
    raw, _ = random_fxp8_matrix_with_real(rng, shape)
    return raw


def random_fxp16_matrix(rng: np.random.Generator, shape=(FULL_SIZE, FULL_SIZE)) -> np.ndarray:
    raw, _ = random_fxp16_matrix_with_real(rng, shape)
    return raw
...
def build_dual_tc_safe_experiment(rng: np.random.Generator, max_attempts: int = 20000):
    for _ in range(max_attempts):
        A, A_original_real = random_fxp8_matrix_with_real(rng)
        B, B_original_real = random_fxp8_matrix_with_real(rng)
        C, C_original_real = random_fxp16_matrix_with_real(rng)

        # Exact hardware-correctness reference:
        # raw-domain fixed-point math from the quantized operands.
        D_full_one_shot = fxp8fxp16_matmul_add(A, B, C)
        if not is_safe_fxp16_matrix(D_full_one_shot):
            continue

        # DPU-error-analysis-style reference:
        # full-precision Python reference from original real operands before
        # fixed-point quantization, then separately rounded to fxp16 for exact checks.
        ...
        D_full_precision_real_reference_rounded = quantize_real_matrix_to_fxp16_raw(
            D_full_precision_real_reference
        )
        if not is_safe_fxp16_matrix(D_full_precision_real_reference_rounded):
            continue
...
def write_human_file(f, exp):
    ...
    f.write("#DPU-like error reference is also written from original random real operands before input quantization.\n")
    ...
    f.write("#FULL_D_16x16_full_precision_real_reference_rounded_fxp16 encoded\n")
    f.write(matrix_to_lines_hex_fxp16(exp["D_full_precision_real_reference_rounded"]) + "\n")
