...
OUTPUT_FILE_HUMAN = os.path.join(
    SCRIPT_DIR,
    "hmma_8instr_dualTC_4octects_fp16_single_experiment.txt"
)
OUTPUT_FILE_TB = os.path.join(
    SCRIPT_DIR,
    "hmma_8instr_dualTC_4octects_fp16_single_experiment_tb_input.txt"
)
...
def build_dual_tc_safe_experiment(rng: np.random.Generator, max_attempts: int = 20000):
    for _ in range(max_attempts):
        ...
        ]).reshape(16, 16).astype(np.float16)

        if not is_safe_fp16_matrix(D_full_from_2tc):
            continue
    ...
    raise RuntimeError("Could not generate a safe dual-TC FP16 16x16 experiment.")
...
def write_human_file(f, exp):
    f.write("#Full 16x16 GEMM-ACC decomposed across 2 tensor cores, each with 2 octects (FP16)\n")
    ...
    f.write("\n#FINAL_CHAINED_16x8_FROM_TC0_FP16\n")
...
def write_tb_file(f, exp):
    f.write("#register file ports content for FP16 dual tensor-core wrapper\n")
