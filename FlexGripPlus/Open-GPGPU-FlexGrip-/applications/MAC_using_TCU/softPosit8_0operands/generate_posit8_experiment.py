...
OUTPUT_FILE_HUMAN = os.path.join(
    SCRIPT_DIR,
    "hmma_8instr_dualTC_4octects_posit8_single_experiment.txt"
)
OUTPUT_FILE_TB = os.path.join(
    SCRIPT_DIR,
    "hmma_8instr_dualTC_4octects_posit8_single_experiment_tb_input.txt"
)
...
# From the user's RTL:
#   PositDecoder_8_0_*
# so this flow targets posit<8,0>.
POSIT_NBITS = 8
POSIT_ES = 0
POSIT_NAR = 1 << (POSIT_NBITS - 1)
POSIT_MASK = (1 << POSIT_NBITS) - 1

# Same modeling choice used in the posit16 script:
# compute in wider precision, round once to posit8 at the end.
...
def posit_bits_to_float(ui: int, nbits: int = POSIT_NBITS, es: int = POSIT_ES) -> float:
    ...
    if idx < len(bits):
        idx += 1  # skip regime termination bit if present
...
def posit8_quantize_scalar(x: float) -> float:
...
def posit8_encode_hex(x: float) -> str:
    bits = float_to_posit_bits(float(x), POSIT_NBITS, POSIT_ES)
    return f"{bits:02X}"


def format_posit8_value(x: float) -> str:
    return f"{float(x):.6g}"


def matrix_to_lines_decimal(mat: np.ndarray) -> str:
    return "\n".join(
        " ".join(format_posit8_value(v) for v in row)
        for row in mat
    )
...
def matrix_to_lines_hex(mat: np.ndarray) -> str:
    return "\n".join(
        " ".join(posit8_encode_hex(float(v)) for v in row)
        for row in mat
    )


def quantize_matrix_posit8(mat: np.ndarray) -> np.ndarray:
    out = np.empty(mat.shape, dtype=np.float64)
    it = np.nditer(mat, flags=["multi_index"])
    for x in it:
        out[it.multi_index] = posit8_quantize_scalar(float(x))
    return out


def random_posit8_matrix(
    ...
    return quantize_matrix_posit8(raw)


def is_safe_posit8_matrix(mat: np.ndarray) -> bool:
...
def posit8_matmul_add(A: np.ndarray, B: np.ndarray, C: np.ndarray) -> np.ndarray:
    ...
    return quantize_matrix_posit8(result_fp64)
...
def write_set_block(
    ...
    for name, mat in (
        ("STEP0_D00", D00), ("STEP0_D10", D10),
        ("STEP1_D01", D01), ("STEP1_D11", D11),
        ("STEP0_D20", D20), ("STEP0_D30", D30),
        ("STEP1_D21", D21), ("STEP1_D31", D31),
    ):
        f.write(f"#golden {prefix} {name}_set{set_idx} decoded\n")
        f.write(matrix_to_lines_decimal(mat) + "\n")
        f.write(f"#golden {prefix} {name}_set{set_idx} encoded\n")
        f.write(matrix_to_lines_hex(mat) + "\n")


# ============================================================
# Testbench file writers
# Same packing idea as the FP8 script:
# pack a whole 1x4 row of posit8 values into one 32-bit portA.
# ============================================================
def encode_row_posit8_hex(row: np.ndarray):
    return [posit8_encode_hex(float(v)) for v in row]


def pack_row_into_ports_posit8(row: np.ndarray):
    """
    row = [v0, v1, v2, v3]

    pack 4 posit8 values into one 32-bit port:
      portA = enc(v3) + enc(v2) + enc(v1) + enc(v0)

    portB stays zero.
    """
    h = encode_row_posit8_hex(row)
...
def write_tb_matrix_block(f, label: str, mat: np.ndarray, base_lane: int):
    ...
    for i in range(4):
        lane_id = base_lane + i
        portA, portB = pack_row_into_ports_posit8(mat[i])
...
def build_one_tc_safe_experiment(
    ...
    for s in range(4):
        ...
        D00 = posit8_matmul_add(A00, B_left_full, C00)
        D10 = posit8_matmul_add(A10, B_left_full, C10)
        D01 = posit8_matmul_add(A00, B_right_full, C01)
        D11 = posit8_matmul_add(A10, B_right_full, C11)

        D20 = posit8_matmul_add(A20, B_left_full, C20)
        D30 = posit8_matmul_add(A30, B_left_full, C30)
        D21 = posit8_matmul_add(A20, B_right_full, C21)
        D31 = posit8_matmul_add(A30, B_right_full, C31)
...
def build_dual_tc_safe_experiment(rng: np.random.Generator, max_attempts: int = 20000):
    for _ in range(max_attempts):
        A = random_posit8_matrix(rng)
        B = random_posit8_matrix(rng)
        C = random_posit8_matrix(rng)

        D_full_one_shot = posit8_matmul_add(A, B, C)

        if not is_safe_posit8_matrix(D_full_one_shot):
            continue
        ...
        for tc in (tc0, tc1):
            for s in range(4):
                ...
                for M in (
                    ...
                    if not is_safe_posit8_matrix(M):
                        ok = False
                        break
                if not ok:
                    break
        ...
        if not is_safe_posit8_matrix(D_full_from_2tc):
            continue
    ...
    raise RuntimeError("Could not generate a safe dual-TC posit8 16x16 experiment.")
...
def write_human_file(f, exp):
    f.write("#Full 16x16 GEMM-ACC decomposed across 2 tensor cores, each with 2 octects (posit8)\n")
    f.write(f"#Configured as posit<{POSIT_NBITS},{POSIT_ES}>.\n")
    f.write("#Golden math computes A @ B + C in float64 and rounds once to posit8 per 4x4 output block.\n")
    ...
    f.write("\n#FINAL_CHAINED_16x8_FROM_TC0_POSIT8\n")
...
def write_tb_file(f, exp):
    f.write("#register file ports content for posit8 dual tensor-core wrapper\n")
    f.write(f"#Configured as posit<{POSIT_NBITS},{POSIT_ES}>.\n")
    f.write("#Each 32-bit portA contains one packed 1x4 posit8 row\n")
...
def main():
    ...
    if GENERATE_TB_INPUT:
        ...
        # Remove stale TB input file from previous runs so the folder reflects
        # the current automated FlexGrip flow.
