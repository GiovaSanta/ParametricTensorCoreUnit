...
entity pipeline_decode is
  ...
  port(
    ...
    hmma_step             : out std_logic;
    w16_out               : out std_logic --continue from here (for completion of widthSel selection for dual tensor core unit signal)
...
architecture arch of pipeline_decode is
  ...
  signal hmma_step_i  : std_logic ;
  signal w16_i        : std_logic ;
...
begin
  ...
  hmma_step_i <= instruction_in(60) when (instr_opcode_b = "0101") ; --hmma instruction related -gio
  ...
  instr_src2_const_i <= instruction_in(24) when (instr_opcode_i = ALU and alu_opcode_i = IADD and instr_is_long_b = '1' and instr_marker_i /= IMM)
                        ...
                        else instruction_in(54) when ((instr_opcode_i = ALU) and ((alu_opcode_i = FMUL) or (alu_opcode_i = FMAD) or (alu_opcode_i = FSET) or (alu_opcode_i = AND_OP)) and (instr_marker_i /= IMM) and (instr_is_long_b = '1'))
  ...
  instr_opcode_i <= MOV when (((instr_opcode_b = "0000") or (instr_opcode_b = "0001") or ((instr_opcode_b = "1101") and (instr_subop_b /= "000"))) and (instr_is_flow_b = '0'))
                    else ALU  when (((instr_opcode_b = "0010") or (instr_opcode_b = "0011") or (instr_opcode_b = "0100") or (instr_opcode_b = "0110") or (instr_opcode_b = "1010") or ((instr_opcode_b = "1101") and (instr_subop_b = "000")) or (instr_opcode_b = "0101")) and (instr_is_flow_b = '0')) --added HMMA case (instrocpde = 0101) (gio)
                    --ADDED FPU
  ...
  alu_opcode_i <= IADD when ((instr_opcode_b = "0010") and (instr_is_flow_b = '0'))
                  ...
                  --HMMA related added by gio
                  else FHMMA                   when ((instr_opcode_b = "0101") and (instr_subop_b = "000"))
...
--else instruction_in(59) when ((instr_opcode_i = ALU) and ((alu_opcode_i = SHL) or (alu_opcode_i = SHL) or (alu_opcode_i = CVT)))
  ...
  w32_i <= instruction_in(22) when ((instr_opcode_i = ALU) and (alu_opcode_i = IMUL24) and (instr_marker_i = IMM))
           ...
           else instruction_in(55) when ((instr_opcode_i = ALU) and ((alu_opcode_i = FHMMA) or (alu_opcode_i = PosHMMA) or (alu_opcode_i = IHMMA) or (alu_opcode_i = FXPHMMA))) --added gio hmma 
  ...
  w16_i <= instruction_in(54) when (instr_opcode_b = "0101")  --added for width operand selection for hmma instr fp8 or fp16 distinction.
           else '0' ;
...
--else x"000000" & "0" & instruction_in(52 downto 46) when ((instr_opcode_i = ALU) and (alu_opcode_i = IADD) and (instr_is_long_b = '1') and (instr_marker_i /= IMM))
            ...
            --added hmma (gio)
            else x"000000" & "0" & instruction_in(22 downto 16)  when ((instr_opcode_i = ALU) and (instr_is_long_b = '1')) --applies also to hmma
...
--else x"0000" & "00" & instruction_in(22 downto 9) when (((instr_opcode_i = MOV) and (mov_opcode_i = LOAD)) and (mov_mem_type_i = MEM_CONST))
  ...
  dest_i <= x"000000" & "00" & instruction_in(7 downto 2) when ((instr_opcode_i = ALU) and (instr_is_long_b = '0'))
            ...
            else x"000000" & "0" & instruction_in(8 downto 2)  when ((instr_opcode_i = ALU) and (instr_is_long_b = '1')) -- applies to hmma as well (gio)
