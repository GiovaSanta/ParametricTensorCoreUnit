...
architecture p_exec_arch of pipeline_execute is
  ...
  --TCU related , gio
  signal start_tcu_i            : std_logic;      
  signal load_tcuBuf_en_i       : std_logic;
  signal load_tcuBuf_ph_i       : std_logic;
  signal load_tcuBuf_pair_i     : std_logic;
  signal exec_tcu_step_i        : std_logic;
  signal busy_tcu_i             : std_logic;
    ...
    if (reset = '1') then
      ...
      hmmaSRCs <= (others => 
        ...
        when IDLE =>
          ...
          if(pipeline_read_done = '1') then
            ...
            if (instr_opcode_in = ALU) then
              ...
              start_tcu_i <= tcu_signal_start; --tcu start
        ...
        when ALU_EXEC =>
          ...
          elsif (tcu_signal_start = '1') then -- double parametric TCU gio
            if(step_done_tcu_i = '1' ) then
              pipeline_execute_state_machine <= ALU_DONE;
            else
              pipeline_execute_state_machine <= ALU_EXEC;
            end if;
...
uSpecialFunctionUnitProcessor : sfu_proc
    ...
    --parametric tcu related

doubleParametricTCUmodule : dualTensorCoreWrapper
  generic map(
    REG_W  => 32,
    ELEM_W => 32
  )
