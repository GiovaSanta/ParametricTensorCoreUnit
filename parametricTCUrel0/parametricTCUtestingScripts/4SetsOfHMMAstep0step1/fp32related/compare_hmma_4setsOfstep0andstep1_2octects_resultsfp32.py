...
def parse_sim_output(path: str):
    ...
    while i < len(lines):
        ...
        if line.upper() in ("#FINAL_16X8_RESULT", "#FINAL_CHAINED_16X8_FROM_STAGED_FP32"):
            matrix, i = read_matrix(lines, i, 16)
            results["final_16x8"] = matrix
            continue

        i += 1

    if not results["sets"] and results["final_16x8"] is None:
        raise ValueError(
            f"No simulator sections recognized in file:\n{path}\n"
            "Expected markers like '#SET 0 results' or '#FINAL_16x8_RESULT'."
        )
...
def parse_golden_file(path: str):
    ...
    results = {
        "sets": {},
        "final_blocks": {},
        "final_16x8_staged": None,
        "final_16x8_ideal": None,
    }
    ...
    while i < len(lines):
        ...
        m = re.match(
            r"#golden\s+(STEP0_D00|STEP0_D10|STEP1_D01|STEP1_D11|STEP0_D20|STEP0_D30|STEP1_D21|STEP1_D31)_set(\d+)\s+encoded",
            line,
            re.IGNORECASE,
        )
        if m:
        ...
        m = re.match(
            r"#(D00_final|D01_final|D10_final|D11_final|D20_final|D21_final|D30_final|D31_final)\s+encoded",
            line,
            re.IGNORECASE,
        )
        if m:
        ...
        if line.upper() == "#D_EXPECTED_TOP_LEFT_16X8 ENCODED":
            matrix, i = read_matrix(lines, i, 16)
            results["final_16x8_ideal"] = matrix
            continue

        i += 1

    needed = (
        "D00_FINAL", "D01_FINAL", "D10_FINAL", "D11_FINAL",
        "D20_FINAL", "D21_FINAL", "D30_FINAL", "D31_FINAL",
    )
    ...
    if all(k in results["final_blocks"] for k in needed):
        ...
        d20 = results["final_blocks"]["D20_FINAL"]
        d21 = results["final_blocks"]["D21_FINAL"]
        d30 = results["final_blocks"]["D30_FINAL"]
        d31 = results["final_blocks"]["D31_FINAL"]

        final_16x8 = []
        for r in range(4):
            final_16x8.append(d00[r] + d01[r])
        for r in range(4):
            final_16x8.append(d10[r] + d11[r])
        for r in range(4):
            final_16x8.append(d20[r] + d21[r])
        for r in range(4):
            final_16x8.append(d30[r] + d31[r])

        results["final_16x8_staged"] = final_16x8

    if not results["sets"] and results["final_16x8_staged"] is None and results["final_16x8_ideal"] is None:
        raise ValueError(
            f"No golden sections recognized in file:\n{path}\n"
            "Expected markers like '#golden STEP0_D00_set0 encoded', "
            "'#D00_final encoded', or '#D_expected_top_left_16x8 encoded'."
        )
        ...
        for c in range(cols):
            ...
            if diff > tolerance:
                ...
                write_line(
                    out,
                    f"{r:>3} {c:>3} | "
                    f"{sim_hex:>10} {sim_val:>14.6f} | "
                    f"{gold_hex:>10} {gold_val:>14.6f} | "
                    f"{diff:>12.6f}"
                )
...
if __name__ == "__main__":
    sim_output_file = r"C:\Users\giovi\OneDrive\Desktop\Magistrale\Tesi\parametricTCUrel0\parametricTCUtestingScripts\4SetsOfHMMAstep0step1\fp32related\hmma_8instr_2octects_tb_output_ctrl_fp32.txt"
    golden_file = r"C:\Users\giovi\OneDrive\Desktop\Magistrale\Tesi\parametricTCUrel0\parametricTCUtestingScripts\4SetsOfHMMAstep0step1\fp32related\hmma_8instr_2octects_fp32_single_experiment.txt"
    report_file = r"C:\Users\giovi\OneDrive\Desktop\Magistrale\Tesi\parametricTCUrel0\parametricTCUtestingScripts\4SetsOfHMMAstep0step1\fp32related\hmma_8instr_2octects_fp32_validation_report.txt"
    ...
    print("Simulator final 16x8 present     :", sim_data["final_16x8"] is not None)
    print("Golden staged final 16x8 present :", golden_data["final_16x8_staged"] is not None)
    print("Golden ideal final 16x8 present  :", golden_data["final_16x8_ideal"] is not None)

    with open(report_file, "w", encoding="utf-8") as out:
        write_line(out, "HMMA 4-SET STEP0/STEP1 FP32 VALIDATION REPORT (2 OCTECTS)")
        ...
        staged_diffs, staged_missing = compare_named_matrix(
            "FINAL 16x8 RESULT vs FINAL_CHAINED_16x8_FROM_STAGED_FP32",
            sim_data["final_16x8"],
            golden_data["final_16x8_staged"],
            out,
            tolerance=tolerance,
        )

        ideal_diffs, ideal_missing = compare_named_matrix(
            "FINAL 16x8 RESULT vs FINAL_IDEAL_16x8_ONE_SHOT_REFERENCE",
            sim_data["final_16x8"],
            golden_data["final_16x8_ideal"],
            out,
            tolerance=tolerance,
        )
