...
OUTPUT_FILE_HUMAN = os.path.join(
    SCRIPT_DIR,
    "hmma_8instr_2octects_fp16_single_experiment.txt"
)
OUTPUT_FILE_TB = os.path.join(
    SCRIPT_DIR,
    "hmma_8instr_2octects_fp16_single_experiment_tb_input.txt"
)
...
def build_two_octect_safe_experiment(rng: np.random.Generator, max_attempts: int = 20000):
    for _ in range(max_attempts):
        ...
        for s in range(4):
            ...
            # octect 0 golden
            D00 = fp16_matmul_add(A00, B_left_full, C00)
            D10 = fp16_matmul_add(A10, B_left_full, C10)
            D01 = fp16_matmul_add(A00, B_right_full, C01)
            D11 = fp16_matmul_add(A10, B_right_full, C11)

            # octect 1 golden
            D20 = fp16_matmul_add(A20, B_left_full, C20)
            D30 = fp16_matmul_add(A30, B_left_full, C30)
            D21 = fp16_matmul_add(A20, B_right_full, C21)
            D31 = fp16_matmul_add(A30, B_right_full, C31)
        ...
        if not is_safe_fp16_matrix(D_chained_top_left_16x8):
            continue
    ...
    raise RuntimeError("Could not generate a safe two-octect FP16 16x16 experiment.")
...
def write_human_file(f, exp):
    f.write("#Single 16x16 GEMM-ACC experiment decomposed into 4 HMMA step0/step1 sets (FP16, 2 octects)\n")
    ...
    f.write("\n#FINAL_CHAINED_16x8_FROM_STAGED_FP16\n")
...
def write_tb_file(f, exp):
    f.write("#register file ports content for FP16, 2 octects\n")
    f.write("#<portA> <portB>\n")
