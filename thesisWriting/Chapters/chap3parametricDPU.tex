...
For the floating-point variants, FloPoCo allowed the exponent and fraction widths to be selected according to the target precision. This made it possible to generate operators for reduced-precision arithmetic which involved 8 bit operands, as well as generating more common circuits supporting more bits in their operands. The generated components were then manually interconnected to construct complete DPU variants implementing the computational model defined in Equation~\eqref{eq:dpu_computational_model}.
...
After the standalone verification of each individual format-specific DPU variant, the validated datapaths were integrated into a common top-level architecture. The objective of the wrapper was to build a first release of the parametric DPU by grouping the different DPU implementations inside a single module and allowing the desired numerical format to be selected through dedicated control signals.

The top-level parametric DPU wrapper does not replace the internal arithmetic structure of the individual DPUs. Instead, it instantiates the previously developed format-specific datapaths and selects the output of the appropriate unit according to the selected format and operand configuration. In this way, the wrapper provides a unified interface while preserving the arithmetic behavior of each individual implementation.

This design choice separates the problem of numerical-format implementation from the problem of format selection. Each internal DPU remains responsible for computing the dot-product operation using its own arithmetic representation and datapath organization. The top-level wrapper is instead responsible for routing the input operands to the available DPUs and selecting the output corresponding to the active format.

The overall organization of the top-level wrapper is shown in Figure~\ref{fig:parametricDPU}. The same input operands are provided to the available format-specific datapaths, while the format-selection logic determines which result is forwarded to the final output.

\begin{figure}[htbp]
\centering
\includegraphics[width=1\textwidth]{Figures/ParametricDPUrel0.pdf}
\caption{Top-level parametric DPU wrapper.}
\label{fig:parametricDPU}
\end{figure}

As shown in the figure, the top-level wrapper groups the available format-specific DPU families inside a single architectural block. The format type and operand width selection signals determine which datapath is active for the current operation and which result is forwarded by the multiplexer. Therefore, the parametric behavior of the module is obtained through selection and multiplexing logic, while the arithmetic computation itself remains delegated to the corresponding format-specific DPU.

\clearpage
\section{Summary}

This chapter described the design and validation flow adopted for the first realease of the parametric Dot Product Unit. The chapter first introduced the common computational model implemented by all DPU variants, based on a four-product dot-product operation with an additional accumulation input. This model was used as a common functional reference for all the numerical formats considered in the thesis.

The chapter then described the construction of the format-specific DPU variants. Floating-point, integer, fixed-point, posit, and LNS datapaths were developed independently, according to the arithmetic operators available for each representation. Some variants were implemented using an early-accumulation structure based on multiply-accumulate stages, while others were implemented using a multiplier-and-adder-tree organization. These design decisions for the first release of the DPU were made based on what was available by the FloPoCo library. But in all cases, the objective was to preserve the same dot-product behavior while changing the underlying numerical representation and datapath organization.

After their construction, the individual DPU variants were verified through standalone simulation testbenches. Random input vectors were generated, applied to each DPU, and compared against expected reference results. This step made it possible to validate each datapath in isolation before integrating the different implementations into a common architecture.

Finally, the chapter presented the top-level parametric DPU wrapper. The wrapper groups the available format-specific datapaths inside a single module and uses format and width selection signals to determine which result is forwarded to the output. Therefore, the parametric behavior of the first DPU release is obtained through selection and multiplexing logic, while the arithmetic computation itself remains delegated to the corresponding format-specific DPU. This architecture represents the fundamental computational building block used in the next stage of the work. In the next chapter, the higher-level Tensor Core Unit architecture design flow is presented, showing how the parametric DPU can be used as a core component to support matrix-oriented operations.